...
Run `marionette mcp-config` inside a configured project to print the bound lead's scoped command. Use `--agent all --format command` to print Codex, Claude Code, and AGY commands plus the oh-my-pi merge instructions for the same binding. Use `--project DIR` to select another binding. Codex alone supports `--format toml`. Outside a configured project, plain `mcp-config` prints the legacy unbound Codex TOML configuration.
...
If setup is needed, `marionette setup --schema` and `setup --help` describe the installed options. A new project uses the shared Herdr `default` session and its own workspace. Pass `--session NAME` only when the user explicitly selected a named session. Setup's `--dry-run` previews its effects. Preserve an existing installation and saved lead. A reachable legacy named-session endpoint resumes unchanged. If the endpoint is missing, `setup` or `lead` can reconnect the project through `default` while preserving its identity, lease, authority, and history. Active tasks, unfinished operations, and Herdr-bound or reserved waits block that repair. An explicit `setup --session default` migration also stops while the original workspace exists; stop its lead and close that workspace first. If the supervisor lacks the safe reconnect protocol, upgrade and restart it before retrying. These failures leave the binding and credentials unchanged and create no replacement workspace.

The setup wizard offers shared, project-local, and custom state directories. Use `--home .marionette` for project-local state. This state contains tokens and absolute paths, so keep it out of Git. For scripted setup, `--install-skills` installs this full skill tree in both `.agents/skills/marionette` and `.claude/skills/marionette`. `--yes` alone does not install tools or skills. Use `--no-install-skills` to state an explicit skip. Existing customized skill files remain unchanged.

`marionette update --check` reports available updates; `update` and `upgrade` update the shared instance and its project bindings and MCP registrations, with rollback on startup failure. Setup offers migration for an older saved runtime; scripted setup requires `--upgrade`. One installed command can address several `--home` instances. Each home owns one supervisor and database and may serve several projects. An existing MCP process keeps its loaded binary and catalog, so restart or reconnect it in the client after those change. Running leads keep their guard files; a newly permitted method needs a new lead launch unless the existing guard already allows it. Marionette retains immutable runtime directories while bindings, registrations, running conversations, or rollback can refer to them. Workspace trust is the general `trustWorkspaces` option for Codex, Claude Code, and AGY; `--no-trust-workspaces` retains native workspace prompts without changing tool approvals.

Harness launch access is configured through setup's `agentAccess` map or an administrative CLI client. Each harness accepts `inherit`, the default, or `full-access` for future terminal sessions. The scoped lead cannot change its coordinator guard or launch access. Existing sessions and Codex desktop app permissions are unchanged; host-managed restrictions still apply. See the README's Harness access section for configuration examples.
...
If the user already supplied the objective, start it; do not create an intake outcome merely to receive a message. `task_submit` without `outcomeId` atomically creates an outcome when no prior authority record is required. For documentation, implementation, or supervisor command checks under coordinator enforcement, create the outcome first, record its authority, then submit the task at the returned revision. Use a strategy only when the work needs one.

Read the project's `authorityMode` before write or command execution. In `conversation` mode, call `authority_record_user_request` with the actual user instruction, current outcome revision, permitted activities, and paths inside the outcome scope. The record is the lead's interpretation of the conversation, not independently authenticated approval. In `external` mode, the user records authority with `marionette authorize`. Projects without a configured mode retain external authority. Planning requests, skill instructions, and intent amendments grant no execution authority.

Use `lead_preferences_get` before `lead_preferences_set`. Save only an available exact profile for the configured lead adapter, supported reasoning, additional instructions, and existing `.agents/skills/*/SKILL.md` or `.claude/skills/*/SKILL.md` references. Model and reasoning changes apply on the next new launch. Repeating `marionette lead` preserves the current native conversation and model. Read updated instructions and skills in the current conversation. To edit a skill file, delegate an authorized task with that path in scope. Preferences cannot change runtime binaries, instance settings, coordinator enforcement, or authority. A supervisor that reports `unknown_action` for preference reads predates this API; continue with empty preferences and upgrade it before reading or saving preferences. Preserve all authentication, permission, and other errors.

Submit an assignment with a stable idempotency `key`, an objective, `kind` (`codex`, `claude`, `agy`, or `omp`), bounded `ownership`, and meaningful `checks`. See [assignment and outcome examples](references/coordination.md) when constructing inputs.
