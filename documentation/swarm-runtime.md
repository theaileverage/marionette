...
Wait results have durable IDs. `swarm.watch.ack` takes `watchId` and exact `resultId`; `swarm.watch.cancel` takes `watchId` and `reason`. A cancelled probe cannot overwrite cancellation when it finishes. Interrupted probes return to waiting after restart and may run again; command authors are responsible for ensuring this repetition is safe. Register `lead.wait` with `condition.watchIds` or `condition.decisionIds` to react even if a result was captured before registration. A ready, failed or cancelled watch wakes the lead to inspect the result; it does not imply success. Conditions retain existing OR behavior across condition groups, and all IDs within each watch/decision group must be ready.
