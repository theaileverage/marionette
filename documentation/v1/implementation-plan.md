...
Native conversation reference inspection is implemented as a separate authorized slice. Migration 007 retains typed, append-only reference observations and legacy bytes; runtime observation supports identity-fenced late enrichment. `attempt.retained-work` / `inspectRetainedWork` exposes typed references, bounded supported history, and job-scoped retained result/artifact provenance. Pi/OMP JSONL and Codex app-server `thread/read` are fixture-verified only; no live user session was used. See [Native session references](native-session-references.md).
