...
function requireString(record, field) {
  ...
  if (typeof value !== 'string' || value.length === 0) fail(`Manifest ${field} is invalid`);
  return value;
}
...
function loadManifest() {
  ...
  if (
    ...
    !verification.argv.every((item) => typeof item === 'string')
  ) {
  ...
  for (const field of ['device', 'inode', 'birthtimeMs', 'protocol']) {
    if (typeof endpoint[field] !== 'number' || !Number.isFinite(endpoint[field])) {
...
async function observeIfBound(client, attempt) {
  const hasIdentity =
    typeof attempt.nativeKind === 'string' &&
    typeof attempt.nativeServerGeneration === 'string' &&
    typeof attempt.nativeLocator === 'string';
