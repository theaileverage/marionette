...
export class Watcher {
  ...
  private constructor(options: WatcherOptions) {
    ...
    this.#deliveryPort =
      'checkReadyEffect' in options.deliveryPort && 'deliverEffect' in options.deliveryPort
        ? options.deliveryPort
        : {
            checkReadyEffect: (input) =>
              promise('Watcher.checkReady', () =>
                (options.deliveryPort as DeliveryPromisePort).checkReady(input),
              ),
            deliverEffect: (input) =>
              promise('Watcher.deliver', () =>
                (options.deliveryPort as DeliveryPromisePort).deliver(input),
              ),
          };
    this.#livenessPort =
      'confirmAbsentEffect' in options.livenessPort
        ? options.livenessPort
        : {
            confirmAbsentEffect: (input) =>
              promise('Watcher.confirmAbsent', () =>
                (options.livenessPort as OwnerLivenessPromisePort).confirmAbsent(input),
              ),
          };
    ...
    function* (this: Watcher) {
      ...
      return yield* Effect.gen(function* () {
        ...
        const readinessResult = yield* Effect.matchEffect(
          self.readiness({ project: self.project, recipient }),
          {
            onFailure: (error) =>
              sync('Watcher.readinessFailed', () =>
                self.settle(rows, 'unconfirmed', `readiness check failed: ${error.message}`),
        ...
        if (readiness.kind === 'unconfirmed') {
          yield* sync('Watcher.readinessUnconfirmed', () =>
            self.settle(rows, 'unconfirmed', readiness.reason),
          );
          return 0;
        }
        if (readiness.kind === 'unsupported') {
          yield* sync('Watcher.readinessUndeliverable', () =>
            self.settle(rows, 'undeliverable', readiness.reason),
          );
          return 0;
        }
            ...
            ),
          {
            onFailure: (error) =>
              sync('Watcher.deliveryUnconfirmed', () => {
                self.settle(rows, 'unconfirmed', error.message);
                return { kind: 'unconfirmed', reason: error.message } as DeliverySubmission;
              }),
        ...
        yield* sync('Watcher.settleDelivery', () => {
          if (result.kind === 'submitted') self.settle(rows, 'submitted');
          else if (result.kind === 'unsupported') self.settle(rows, 'undeliverable', result.reason);
          else self.settle(rows, 'unconfirmed', result.reason);
        });
...
export interface WatcherServiceShape {
...
export class WatcherService extends Context.Service<WatcherService, WatcherServiceShape>()(
