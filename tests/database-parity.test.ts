...
type ErrorResult =
  ...
  | { kind: 'failure'; name: string; message: string };

function primitiveTypeName(cause: unknown): string {
...
function errorResult<Value>(encode: (value: Value) => string, value: Value): ErrorResult {
  try {
    return { kind: 'success', value: encode(value) };
  } catch (error) {
    return {
      kind: 'failure',
      name: error instanceof Error ? error.name : primitiveTypeName(error),
