...
test('scope interruption after a durable launch claim never invokes launch twice', async (t) => {
  ...
  const effectCount = Schema.decodeUnknownSync(Schema.Struct({ count: Schema.Finite }))(effects);

  assert.equal(effectCount.count, 1);
});
...
function watcherFixture(t: TestContext) {
  ...
  const wakeRow = Schema.decodeUnknownSync(Schema.Struct({ subscription_id: Schema.String }))(wake);

  return { store, wakeId: wakeRow.subscription_id };
}
...
test('scope close marks a claimed delivery uncertain and a replacement never replays it', async (t) => {
  ...
  assert.equal(
    fixture.store.read(
      (db) =>
        Schema.decodeUnknownSync(Schema.Struct({ state: Schema.String }))(
          db
