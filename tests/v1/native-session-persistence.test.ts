...
test('reopen retains native references and result artifact provenance', (t) => {
  ...
  const project = {
    id: ProjectIdSchema.parse('native_persistence'),
    hostId: HostIdSchema.parse('host_persistence'),
    repositoryRoot: join(directory, 'repo'),
    stateDirectory: join(directory, 'state'),
  };
  let store = Store.open({ databasePath, project });
  const controller = store.registerSession({
    id: AgentSessionIdSchema.parse('controller-persistence'),
    generation: 1,
    workspaceId: null,
    role: 'controller',
  ...
  const workspaceId = WorkspaceIdSchema.parse('workspace-persistence');
  ...
  const worker = store.registerSession({
    id: AgentSessionIdSchema.parse('worker-persistence'),
    generation: 1,
    workspaceId,
    role: 'worker',
  ...
  const result = store.recordResult({
    ...
    content: {
      ...
      artifactDigests: [DigestSchema.parse(artifact.digest)],
    },
