...
import type { NativeObservation } from '../src/v1/native.js';
...
function reservationState(current: Fixture): string | undefined {
  const row = current.store.read((database) =>
    database
      .prepare('SELECT state FROM execution_reservations WHERE id = ?')
      .get(current.admission.reservationId),
  );

  return row === undefined
    ? undefined
    : Schema.decodeUnknownSync(Schema.Struct({ state: Schema.String }))(row).state;
}
...
test('recovery is controller-authorized', (t) => {
  ...
  assert.throws(
    ...
    (cause: unknown) => cause instanceof StoreError && cause.code === 'permission-denied',
  );
...
test('never-started recovery is idempotent, releases the reservation, and creates no native effect', (t) => {
  ...
  assert.throws(
    ...
    (cause: unknown) => cause instanceof StoreError && cause.code === 'stale-revision',
  );
  assert.equal(reservationState(current), 'held');
  ...
  assert.throws(
    ...
    (cause: unknown) => cause instanceof StoreError && cause.code === 'idempotency-conflict',
  );
});
...
test('recovery refuses a persisted working native observation', (t) => {
  ...
  assert.throws(
    ...
    (cause: unknown) => cause instanceof StoreError && cause.code === 'invalid-state',
  );
...
test('attempt.reconcile dispatches an explicit recovery payload without replaying reconciliation', async () => {
  ...
  // SAFETY: This test invokes only the two overridden Effect methods; the prototype supplies the Marionette method surface.
  const client = Object.assign(Object.create(Marionette.prototype) as Marionette, {
    recoverAttemptEffect: (input: Parameters<Marionette['recoverAttemptEffect']>[0]) => {
      recovered += 1;
