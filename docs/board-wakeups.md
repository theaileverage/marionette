...
The design is identical to the root runtime's, documented at
`documentation/v1/board-wakeups.md`. This page states it once more so the port
reads on its own, and adds the Effect-specific sections at the end.
...
Both are forked fibers in one scope (`Effect.forkChild`), raced against the
abort signal. Closing the scope interrupts whichever is still running and
releases the wake endpoint and then the ownership claim, in that order.
...
The idle timer asks `Watcher.hasPendingWorkEffect(): Effect<boolean, WatcherError>`:
a cheap read that performs no native observation and settles nothing. `false`
must mean it is safe to idle out.
...
- `WakeError` is a `Schema.TaggedError`. Binding failures are classified into it
  and then converted into the listener's `unavailable` reason, so a transport
  that cannot bind never reaches the caller's error channel: an unbindable
  endpoint is a degraded transport, not a failed watch.
- `pokeWatcherEffect` has no error channel at all. It answers `false` rather than
  failing, because a poke that does not land is not an error condition.
- `WakeListener.waitEffect` is built on `Effect.callback` and is interruptible:
  its finalizer clears the timer and detaches the waiter, so an interrupted wait
  consumes no signal and the next wait still sees a poke that arrived meanwhile.
- Each pass body is `Effect.uninterruptible`; only the wait between passes is
  interruptible. A stop therefore lands between passes rather than part-way
  through a native effect.
- The listener and the watcher are both acquired with `Effect.acquireRelease`, so
  the endpoint is removed and ownership is settled on every exit path, including
  interruption and defect.
- `PendingWorkEffectPort<E>` carries its error type so the watcher's own
  `WatcherError` and the transitional `WakeError` reader both satisfy it without
  widening anybody's channel to `unknown`.

## Verifying it

The claims above are tested in `effect-port/tests/wake-effect.test.ts` and
mirrored in the root runtime at `tests/v1/wake.test.ts` and
`tests/v1/watch-lifecycle.test.ts`. Both suites report measured numbers as test
diagnostics rather than asserting performance targets: latency, pass counts and
observation counts are properties of the host that ran them.

While `effect-port/scripts/test.mjs` is blocked by a stale baseline manifest,
this file is run directly with esbuild plus `node --test` against
`effect-port/src`. Do not weaken the baseline verifier to make the wrapper pass.
