...
const migrationInteger = Schema.decodeUnknownSync(
  integer.check(
...
const migrationText = Schema.decodeUnknownSync(Schema.String);

const busyFailure = Schema.decodeUnknownSync(
  Schema.Struct({
    errcode: integer,
  }),
);
...
export class DatabaseOperationError extends Schema.TaggedError<DatabaseOperationError>()(
  'Marionette.DatabaseOperationError',
  {
...
function isBusyFailure(cause: unknown): boolean {
  try {
    return (busyFailure(cause).errcode & 255) === 5;
  } catch {
    return false;
  }
}
  ...
  try {
    ...
    for (;;) {
      try {
        ...
        if (!isBusyFailure(error) || performance.now() >= deadline) throw error;
...
export interface DatabaseService {
  ...
  readonly execute: <A>(
    operation: string,
    action: (database: DatabaseSync) => A,
  ) => Effect.Effect<A, DatabaseOperationError>;
}

export class Service extends Context.Service<Service, DatabaseService>()(
  '@marionette/v1/Database',
) {}

function makeService(database: DatabaseSync): DatabaseService {
  const execute: DatabaseService['execute'] = Effect.fn('Database.execute')(
    <A>(operation: string, action: (database: DatabaseSync) => A) =>
      Effect.try({
        try: () => action(database),
  ...
  return Service.of({ database, execute });
}

export function layer(options: OpenDatabaseOptions): Layer.Layer<Service, DatabaseOperationError> {
  const acquire = Effect.try({
    try: () => openDatabase(options),
    catch: (cause) => new DatabaseOperationError({ operation: 'Database.open', cause }),
  });

  return Layer.effect(
    Service,
    Effect.acquireRelease(acquire, (database) =>
      Effect.sync(() => {
