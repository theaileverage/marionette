...
export type InputRecord = Schema.JsonObject;

const optionValueSchema = Schema.Record(
  Schema.String,
  Schema.optional(Schema.Union([Schema.String, Schema.Boolean])),
);

const inputRecordSchema = Schema.JsonObject;

const outputMode = Schema.Literals(['human', 'json', 'ndjson']);

const decodeOptionValues = Schema.decodeUnknownSync(optionValueSchema);

const decodeInputRecord = Schema.decodeUnknownSync(inputRecordSchema);
...
export function requestedOutput(argv: string[]) {
  try {
    ...
    return Option.getOrUndefined(Schema.decodeUnknownOption(outputMode)(parsed.values.output));
  } catch {
    return undefined;
  }
}
...
export function parseCommand(argv: string[]) {
  try {
    ...
    return { positionals: parsed.positionals, values: decodeOptionValues(parsed.values) };
  } catch {
    throw new CliError(
      'invalid-options',
...
export function stringOption(
  ...
  if (value === undefined || Predicate.isString(value)) return value;
  throw new Error(`Expected --${name} to have a string value`);
}
...
function parseJson(text: string): InputRecord {
  ...
  try {
    return decodeInputRecord(JSON.parse(text));
  } catch {
    throw new CliError(
      'invalid-json',
...
export function operationInput(name: string, values: Record<string, string | boolean | undefined>) {
  ...
  if (input !== undefined) raw = readInput(input);
    ...
    const built: Record<string, Schema.Json> = {};
  ...
  const record = decodeInputRecord(raw);
