...
import { AttemptIdSchema, type AttemptId, type WorkflowId } from '../model.js';
...
export class ServiceControls {
  ...
  constructor(
    ...
    readonly withWorkflowControlAuthority: <T>(
      workflowId: WorkflowId | null,
      input: { attemptId: AttemptId; controlIntentId: string },
      effect: () => T,
    ) => T = (_workflowId, _input, effect) => effect(),
  ) {}
  ...
  async scan(limit = 20) {
    ...
    const rows = store.read((db) => {
      const after = this.#cursor
        ? db
            .prepare(
              "SELECT * FROM attempt_control_intents WHERE project_id=? AND state IN ('requested','unconfirmed') AND (created_at>? OR (created_at=? AND id>?)) ORDER BY created_at,id LIMIT ?",
            )
            .all(
              store.project.id,
      ...
      const wrapped = remaining
        ? this.#cursor
          ...
          : db
              ...
              .all(store.project.id, remaining)
        : [];
      return [...after, ...wrapped].map((row) => controlSchema.parse(row));
    });
    const last = rows.at(-1);
    if (last) this.#cursor = { createdAt: last.created_at, id: last.id };
    for (const control of rows) {
      ...
      const journal: NativeJournal = {
        prepare: async (effect) =>
          this.withWorkflowControlAuthority(
            attempt.workflowId,
            { attemptId: control.attempt_id, controlIntentId: control.id },
            () =>
              store.transaction((db) => {
                this.authorize(db);
                ...
                if (control.cause_kind === 'workflow-control') {
                  ...
                  if (!live.workflowId)
                    return { kind: 'rejected', reason: 'Control workflow missing' };
                  ...
                  if (!cause) {
                    ...
                    if (
                      ...
                      return {
                        kind: 'rejected',
                        reason: 'Control cause missing or obsolete deadline',
                      };
                  } else if (
