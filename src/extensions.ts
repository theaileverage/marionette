...
const resourcePath = Schema.String.check(
  Schema.makeFilter((value) => {
    ...
    return (
      value.length > 0 &&
      value.length <= 4096 &&
      [...value].every((character) => character !== '\\' && character !== ':' && character.charCodeAt(0) > 31) &&
...
export class ExtensionCatalog extends Context.Service<
  ExtensionCatalog,
  {
    ...
    readonly resolve: <Reference>(reference: Reference) => Effect.Effect<ExtensionSnapshot, ExtensionError>;
  }
